...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#18181b;">
  ...
  <p style="margin:0 0 4px;"><strong>PR #${prNumber}: ${prTitle}</strong></p>
  <p style="margin:0 0 20px;"><a href="${prUrl}" style="color:#6366f1;">${prUrl}</a></p>
  ...
  <div style="border-left:3px solid ${severityColor(f.severity)};padding:8px 12px;margin-bottom:8px;background:#fafafa;border-radius:0 4px 4px 0;">
    <code style="font-size:12px;color:#71717a;">${f.file}:${f.line}</code>
    <span style="display:inline-block;background:${severityColor(f.severity)};color:#fff;font-size:11px;padding:1px 6px;border-radius:4px;margin-left:6px;">${f.severity}</span>
    <span style="display:inline-block;background:#e4e4e7;color:#3f3f46;font-size:11px;padding:1px 6px;border-radius:4px;margin-left:4px;">${f.category}</span>
    <p style="margin:6px 0 0;">${f.message}</p>
  </div>`).join('')}` : ''}

  <a href="${prUrl}" style="display:inline-block;margin-top:20px;background:#6366f1;color:#fff;padding:10px 22px;border-radius:6px;text-decoration:none;font-weight:500;">View PR on GitHub →</a>
